...
# sdc--advanced-lane-finding-master\camera_cal
...
def proceesvideo(img):

    img = cv2.undistort(img, mtx, dist, None, mtx)

        # Calculate directional gradient
    grad_binary = abs_sobel_thresh(img, orient='x', sobel_kernel=3, thresh=(30, 100))

        # Calculate gradient magnitude 
    mag_binary = mag_thres(img, sobel_kernel=3, thresh=(50, 100))

        # Calculate gradient direction
    dir_binary = dir_thresh(img, sobel_kernel=3, thresh=(0.7, 1.3))
...
# cap = cv2.VideoCapture('sdc--advanced-lane-finding-master/project_video.mp4')
...
while(cap.isOpened()):
  ...
  if ret == True:
    ...
    out=proceesvideo(frame)
    # resize =cv2.resize(out,(720,1280)) 
    print(out.shape)
